...
export default function Machines() {
  ...
  const openEdit = (m) => { setEditing(m); setNumber(m.number); setName(m.name || ""); setGroup(m.group || "A"); setShowForm(true); };

  const save = async (e) => {
    e.preventDefault();
    try {
      if (editing) {
        await api.patch(`/machines/${editing.id}`, { number, name, group });
      } else {
        await api.post("/machines", { number, name, group });
      }
          ...
          <div className="flex border border-slate-200 overflow-hidden">
            {["all", "A", "B"].map((g) => (
              <button key={g} onClick={() => setGroupFilter(g)}
                ...
                {g === "all" ? (lang === "ar" ? "الكل" : "All") : `Group ${g}`}
              </button>
            ))}
          </div>
