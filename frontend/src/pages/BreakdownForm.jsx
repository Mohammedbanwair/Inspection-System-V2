    ...
    <div>
      ...
      <div className="flex gap-1">
        ...
        <select value={ampm} onChange={(e) => setAmpm(e.target.value)}
                className="w-20 h-11 px-2 border border-slate-200 text-sm bg-white">
...
export default function BreakdownForm({ onBack, onSubmitted }) {
  ...
  const brief = problem === t("other_problem") ? otherDesc.trim() : problem;

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!machineId) { toast.error(ar ? "اختر مكينة" : "Select a machine"); return; }
    if (!brief)     { toast.error(ar ? "اختر أو اكتب وصف العطل" : "Select or enter a problem description"); return; }
    setSaving(true);
    try {
  ...
  if (done) {
    return (
      <div className="max-w-xl mx-auto text-center py-16">
        ...
        <p className="text-sm text-slate-500 mb-8">{ar ? "تم تسليم التقرير إلى الادمن بنجاح." : "The report has been delivered to the admin successfully."}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white border border-slate-200 p-5">
          <div className="grid grid-cols-2 gap-4">
            <TimePicker label={t("start_time")} value={startTime} onChange={setStartTime} />
            <TimePicker label={t("end_time")} value={endTime} onChange={setEndTime} />
          </div>
        </div>
